    ...
    <header className={`absolute top-0 left-0 w-full z-[100] py-6 border-b border-transparent transition-colors ${forceNightModeStyles ? 'bg-transparent' : 'bg-transparent'}`}>
      <div className="max-w-[1400px] w-full mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          {isHomePage && (
            <button id="header-button-58" onClick={toggleTheme}
          ...
          <a id="header-a-59" href="tel:+919788770786" className={`hidden md:inline-flex items-center border-2 px-5 py-2.5 rounded-full text-sm font-bold transition-colors duration-300 ${forceNightModeStyles ? 'bg-white border-black text-black hover:bg-black hover:text-white': 'bg-white border-black text-black hover:bg-black hover:text-white'}`}>
          ...
          <button id="header-button-60" onClick={() => openQuoteModal()} className="hidden sm:block bg-[#ee0000] text-white px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-300 hover:bg-[#cc0000] hover:-translate-y-0.5 hover:shadow-[0_10px_20px_rgba(249,92,25,0.3)] shadow-[0_4px_14px_rgba(249,92,25,0.4)]">
            Get Free Quote
          </button>
          
          <button id="header-button-61" aria-label="Toggle Mobile Menu" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`md:hidden relative z-[110] p-1 ${isMobileMenuOpen ? 'text-[#ee0000]' : (forceNightModeStyles ? 'text-white' : 'text-black')}`}>
      ...
      <div className={`fixed inset-0 bg-white/95 backdrop-blur-md z-[105] transition-transform duration-300 flex flex-col pt-24 pb-8 px-8 md:hidden overflow-y-auto ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <ul className="flex flex-col gap-6 list-none m-0 p-0">
          ...
          <li className="flex flex-col">
            <button id="header-button-81" aria-label="Toggle Community Menu" onClick={() => setIsMobileCommunityOpen(!isMobileCommunityOpen)} className="flex items-center justify-between text-slate-900 no-underline text-2xl font-extrabold hover:text-[#00a3e0] w-full text-left">
              Community
